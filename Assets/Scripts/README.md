...
This directory contains the core scripts for the MR Companion project — a Mixed Reality application that spawns an AI-powered avatar companion on the user's desk using Meta Quest's Scene Understanding (MRUK).
...
AvatarDeskPlacer (entry point)
 ...
 └── CompanionVoiceController (mic, playback, lip sync)
...
- Locates the face mesh (by name or by blendshape search) and wires up `CompanionVoiceController` with an `AudioSource`, face mesh, and mouth blendshape index
...
- `TriggerCompanionInitiation()` — two-step initiation: contextual update + user message to make the agent start talking naturally
...
- `MakeAISpeak(prompt)` — inject a user message and trigger a response (for companion-initiated conversation)
...
UDP server that listens for commands from the experimenter's PC/laptop, enabling remote control of the companion during study sessions. Both devices must be on the same WiFi network.

**Supported commands:**

| Command | Description |
|---|---|
| `INITIATE` | Trigger companion-initiated conversation (default prompt) |
...
**How it works:**
1. Detects the rising edge of `CompanionVoiceController.IsAISpeaking`
