...
public class ExperimenterRemoteTrigger : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (voiceController == null)
            Debug.LogWarning("ExperimenterRemoteTrigger: No CompanionVoiceController found. MIC commands will be unavailable.");
    ...
    private void HandleCommand(string raw)
    {
        ...
        {
            case "INITIATE":
                // Trigger companion to start talking
                if (!string.IsNullOrEmpty(payload))
                {
                    elevenLabs.TriggerCompanionInitiation(context: payload);
                }
                else
                {
                    elevenLabs.TriggerCompanionInitiation();
                }
                SendReply("OK: INITIATE");
                Debug.Log("ExperimenterRemoteTrigger: >>> Companion initiation triggered.");
                break;
    ...
    private void HandleMicCommand(string payload)
    {
        if (voiceController == null)
        {
            SendReply("ERROR: No CompanionVoiceController found");
            return;
        }
