...
/// <summary>
/// Controls the companion's voice interaction: mic capture, audio playback, and lip sync.
...
/// </summary>
public class CompanionVoiceController : MonoBehaviour
{
    ...
    {
        isMicMuted = muted;
        Debug.Log($"CompanionVoiceController: Mic {(muted ? "MUTED" : "UNMUTED")}.");
    }
    ...
    private void Start()
    {
        ...
        if (backend == VoiceBackend.ElevenLabs)
        {
            ...
            if (elevenLabsConnection == null)
            {
                Debug.LogError("CompanionVoiceController: No ElevenLabsConnection found!");
                enabled = false;
                return;
            }
        ...
        else
        {
            ...
            if (openAIConnection == null)
            {
                Debug.LogError("CompanionVoiceController: No RealtimeAPIConnection found!");
                enabled = false;
                return;
            }
    ...
    public void Initialize(AudioSource audioSource, SkinnedMeshRenderer faceMeshRenderer, int mouthBlendIndex)
    {
        ...
        Debug.Log($"CompanionVoiceController: Initialized ({backend}, {sr}Hz, streaming mode).");
    }
    ...
    {
        Debug.Log("CompanionVoiceController: API connected.");
        if (alwaysListening)
        {
            StartListening();
        }
    ...
    public void StartListening()
    {
        ...
        {
            Debug.LogError("CompanionVoiceController: No microphone found!");
            return;
        }

        micDevice = Microphone.devices[0];
        Debug.Log($"CompanionVoiceController: Using microphone: {micDevice}");

        micClip = Microphone.Start(micDevice, true, micBufferLengthSec, sampleRate);
        lastMicPosition = 0;
        isRecording = true;

        Debug.Log("CompanionVoiceController: Mic recording started.");
    }
    ...
    public void StopListening()
    {
        ...
        Debug.Log("CompanionVoiceController: Mic recording stopped.");
    }
    ...
    private void ProcessMicAudio()
    {
        ...
        {
            Debug.Log($"CompanionVoiceController: Mic RMS = {rms:F5}");
        }
    ...
    {
        IsAISpeaking = true;
        Debug.Log("CompanionVoiceController: New agent response detected.");
    }
    ...
    private void OnAudioChunkReceived(string base64Audio)
    {
        ...
        if (isBuffering)
        {
            if (audioQueue.Count >= preBufferSamples)
            {
                ...
                isPlaying = true;
                Debug.Log($"CompanionVoiceController: Pre-buffer filled ({audioQueue.Count} samples). Playback started.");
            }
            return;
        }
        ...
        {
            isBuffering = true;
            Debug.Log("CompanionVoiceController: Buffering audio...");
            ...
            if (audioQueue.Count >= preBufferSamples)
            {
                ...
                avatarAudioSource.Play();
                isPlaying = true;
                Debug.Log("CompanionVoiceController: Playback started.");
            }
        }
    }
    ...
    private void OnAudioResponseDone()
    {
        if (isBuffering && audioQueue.Count > 0)
        {
            ...
            avatarAudioSource.Play();
            isPlaying = true;
            Debug.Log("CompanionVoiceController: Short response — playing without full buffer.");
        }
        ...
        IsAISpeaking = false;
        Debug.Log($"CompanionVoiceController: AI audio stream ended. Added {drainDelaySeconds}s of silence padding.");
    }
    ...
    private void OnInterruption()
    {
        ...
        isBuffering = false;
        IsAISpeaking = false;
        Debug.Log("CompanionVoiceController: Playback interrupted by user.");
    }
    ...
    private void CheckPlaybackCompletion()
    {
        ...
        if (!IsAISpeaking && isPlaying)
        {
            if (audioQueue.IsEmpty)
            {
                avatarAudioSource.Stop();
                isPlaying = false;
                Debug.Log("CompanionVoiceController: Playback completed naturally.");
            }
        }
    }
    ...
    private void CheckAudioTimeout()
    {
        ...
        {
            Debug.Log("CompanionVoiceController: Audio timeout — no more chunks expected.");
            IsAISpeaking = false;
            ...
            if (isBuffering && audioQueue.Count > 0)
            {
                ...
                avatarAudioSource.Play();
                isPlaying = true;
                Debug.Log("CompanionVoiceController: Timeout during buffering — playing partial buffer.");
            }
